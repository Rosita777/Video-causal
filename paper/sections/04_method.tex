...
additionally freeze 36 generic preservation bindings shared by all 18 Wan
method/control training runs. For every selected
...
and timestep. The \ces{} components convert source and footprint visibility
into gated per-output erasure credit. Only the paired \methodshort{}--Matched
...
optimized,
and checkpoint 200 is the sole checkpoint eligible for evaluation. We train a
separate adapter for each mechanism, so \methodshort{} denotes a shared
training principle rather than one universal seven-mechanism adapter. The
Matched and \methodshort{} arms share both objectives, their weights,
counterfactual and preservation data, initialization, schedule, stochastic
draws, optimizer, and checkpoint policy. Their comparison therefore isolates
source-slot replacement, not the necessity of flow matching, teacher
distillation, preservation matching, or LoRA itself. Full optimization and
inference settings appear in \appref{app:optimization-details}.

% Figure 2 is specified in paper/WRITING_PLAN.md and
% paper/figures/main/README.md. Insert the frozen asset only after its source
% manifest and hash exist.
