...
run_part "$GPU_A" prompts/waterdrop_scene_probe30_part_a.txt "$OUTPUT_ROOT/part_a" "$SEED_A" \
  >"$LOG_ROOT/part_a.log" 2>&1 &
pid_a=$!

run_part "$GPU_B" prompts/waterdrop_scene_probe30_part_b.txt "$OUTPUT_ROOT/part_b" "$SEED_B" \
  >"$LOG_ROOT/part_b.log" 2>&1 &
pid_b=$!

echo "part_a_pid=$pid_a gpu=$GPU_A seeds=${SEED_A}-$((SEED_A + 14))"
echo "part_b_pid=$pid_b gpu=$GPU_B seeds=${SEED_B}-$((SEED_B + 14))"
