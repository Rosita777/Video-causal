...
FIXED_SEED="${FIXED_SEED:-9200}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_ROOT/outputs/waterdrop_target_only_resting20_wan}"
LOG_ROOT="${LOG_ROOT:-$PROJECT_ROOT/logs/waterdrop_target_only_resting20_wan}"
...
run_shard() {
  local shard="$1" gpu="$2"
  local prompts="prompts/waterdrop_target_only_resting20_shard_${shard}.txt"
  local seeds
...
run_shard 2 3 & pid2="$!"
run_shard 3 3 & pid3="$!"
echo "gpu=2 shards=0,1; gpu=3 shards=2,3"
