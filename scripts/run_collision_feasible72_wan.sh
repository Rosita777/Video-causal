...
SEEDS="$(tr -d '\n' < "prompts/collision_feasible72_shard_${SHARD}_seeds.txt")"

exec "$PYTHON" scripts/generate_wan_clean.py \
  --baseline clean \
  --prompts "prompts/collision_feasible72_shard_${SHARD}.txt" \
  --output-dir "outputs/collision_feasible72_wan/shard_${SHARD}" \
