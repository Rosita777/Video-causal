...
FIXED_SEED="${FIXED_SEED:-9300}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_ROOT/outputs/waterdrop_target_only_replenish30_wan}"
LOG_ROOT="${LOG_ROOT:-$PROJECT_ROOT/logs/waterdrop_target_only_replenish30_wan}"
...
run_shard() {
  local shard="$1" gpu="$2"
  local prompts="prompts/waterdrop_target_only_replenish30_shard_${shard}.txt"
  local seeds
...
run_shard 4 3 & pid4="$!"
run_shard 5 3 & pid5="$!"
echo "gpu=1 shards=0,1; gpu=2 shards=2,3; gpu=3 shards=4,5"
echo "pids=$pid0 $pid1 $pid2 $pid3 $pid4 $pid5 fixed_seed=$FIXED_SEED"
status=0
