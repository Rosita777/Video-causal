#!/usr/bin/env python3
"""Generate or plan ZeroScope/ModelScope clean and negative-prompt videos."""
...
DEFAULT_OUTPUT_DIR = Path("outputs/zeroscope_clean")
...
def generate_videos(args: argparse.Namespace, items: list[dict[str, object]]) -> None:
    try:
        ...
        raise SystemExit(
            "ZeroScope generation requires torch and diffusers with TextToVideoSDPipeline. "
...
def main() -> int:
    ...
    if args.dry_run:
        print(f"Dry-run ZeroScope manifest written: {manifest}")
    else:
        print(f"ZeroScope generation manifest written: {manifest}")
    return 0
