...
exec "$PYTHON" scripts/train_wan_waterdrop_lora.py \
  ...
  --output-dir outputs/adapters/collision_target_components_union4_50 \
  --role erase --objective target_conditioned_components \
  --target-phrase "red rubber ball" \
  --causal-gate-dir outputs/causal_gates/collision_dual_gate_d2 \
  --activation-gate-dir outputs/component_gates/collision_overfit4 \
  --component-gate-dir outputs/component_gates/collision_overfit4 \
  --gate-floor 0 --max-steps 50 --save-every 25 --learning-rate 1e-4 \
  --rank 16 --alpha 16 --background-weight 4 \
  --pair-weight 0 --redirect-weight 0 --object-weight 4 --receiver-weight 2 \
  --preserve-weight 0
