#!/usr/bin/env python3
"""Run a SAFREE-style ZeroScope/ModelScope inference-time erasure proxy."""
...
from zeroscope_adapter_common import (
    add_common_args,
    base_item,
    encode_cfg,
...
BASELINE = "safree_zeroscope"
LOCAL_METHOD = "safree_zeroscope_proxy_v0"


def implementation_config(args: argparse.Namespace) -> dict[str, object]:
    official_path = args.safree_root / "modelscope" / "pipeline.py"
    return {
        ...
        "notes": (
            "No official SAFREE ZeroScope/ModelScope adapter is present locally. "
            "This proxy keeps the original prompt, uses the target concept as "
            "negative guidance, and subtracts a target-concept embedding direction "
            "from the positive prompt embedding."
        ),
    }
...
def encode_item(pipe, torch_module, selected_device: str, item: dict[str, object]):
    safree = item["safree"]
...
def main() -> int:
    ...
    print(f"ZeroScope {BASELINE} manifest written: {manifest}")
    return 0
