"""Shared helpers for ZeroScope/ModelScope-family baseline adapters."""
...
DEFAULT_MODEL = "models/zeroscope_v2_576w"
PIPELINE_NAME = "TextToVideoSDPipeline"
...
def load_zeroscope_pipe(args: argparse.Namespace):
    try:
        import torch
        ...
        raise SystemExit(
            "ZeroScope adapters require torch and diffusers with TextToVideoSDPipeline."
        ) from exc

    torch_dtype = resolve_torch_dtype(torch, args.dtype)
    pipe = TextToVideoSDPipeline.from_pretrained(args.model, torch_dtype=torch_dtype)
    ...
    return torch, export_to_video, pipe, selected_device


def encode_cfg(pipe, torch_module, *, prompt: str, negative_prompt: str, device: str):
...
def generate_encoded_videos(
    ...
    for item in items:
        prompt_embeds, negative_prompt_embeds = encode_item(pipe, torch_module, selected_device, item)
