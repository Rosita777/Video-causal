...
Current endpoint status on 2026-06-22: the provided `https://api.360.cn/v1` default group lists `openai/gpt-4o`, but real calls return `no available channel`. The implementation therefore supports GPT-4o, while the tracked sample artifact uses `openai/gpt-4o-mini` only as a fallback smoke test. The fallback is not good enough to be the main judge.
